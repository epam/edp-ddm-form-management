...
module.exports = function permissionHandler(router) {
   ...
   */
  const getSubmissionResourceAccess = (req, submission, access, next) => {
    if (!next) {
      return null;
    }
  ...
  const getSubmissionFieldMatchAccess = (req, submission, access) => {
    if (!submission || !access) {
      return;
    }
    ...
    Object.entries(req.submissionFieldMatchAccess).forEach(([permissionLevel, conditions]) => {
      ...
      conditions.forEach((condition) => {
        ...
        if (rolesIntersection.length) {
          const {
            formFieldPath, operator, value, valueType,
          } = condition;
   ...
   *   The compiled access list.
   */
  const getSelfAccessPermissions = (req, form, access) => {
    ...
    for (let a = 0; a < form.submissionAccess.length; a++) {
      ...
      if (hasOwnProperty(form.submissionAccess[a], 'type') && form.submissionAccess[a].type === 'self') {
        done = true;

        // Flag the request for self access, so that the submission permission handler can add it in.
        req.selfAccess = true;

        // Remove the self access type, so we don't disturb the regular _all/_own permissions.
        // eslint-disable-next-line no-param-reassign
  ...
  router.formio.access = {
    ...
    getAccess(req, res, done) {
      ...
      async.series(hook.alter('getAccess', [
        ...
        function setSubmissionFieldMatchAccessToRequest(callback) {
          ...
          return router.formio.cache.loadForm(req, null, req.formId, (err, item) => {
            if (err) {
              return callback(err);
            }
            if (!item) {
        ...
        // Mark the index request to be processed by SubmissionFieldMatchAccessFilter
        function flagIndexRequestAsSubmissionFieldMatchAccess(callback) {
          const isIndexRequest = (request) => request.method.toUpperCase() === 'GET'
            && request.formId
            && !request.subId;
        ...
        function getSubmissionAccess(callback) {
          ...
          return router.formio.cache.loadSubmission(req, req.formId, req.subId, (err, submission) => {
            if (err) {
              return callback(400);
            }
    ...
    //   const typedAccess = function (type) {
  ...
  return (req, res, next) => {
    ...
    // Allow the private hook of skip to be run, if it didn't already pass the whitelist.
    if (!skip) {
    ...
    router.formio.access.getAccess(req, res, (err, access) => {
      ...
      let entity = null;
      if (hasOwnProperty(req, 'subId') && ((req.subId !== null) && (req.subId !== undefined))) {
        entity = {
          type: 'submission',
          id: req.subId,
        };
      } else if (hasOwnProperty(req, 'formId') && ((req.formId !== null) && (req.formId !== undefined))) {
        entity = {
          type: 'form',
          id: req.formId,
        };
      } else if (hasOwnProperty(req, 'roleId') && ((req.roleId !== null) && (req.roleId !== undefined))) {
        entity = {
          type: 'role',
          id: req.roleId,
        };
