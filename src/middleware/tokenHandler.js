...
const debug = {
  error: debugModule('formio:error'),
  handler: debugModule('formio:middleware:tokenHandler'),
};
...
module.exports = (router) => {
  ...
  const formioCache = cacheModule(router);
  const {
    jwt: jwtConfig,
  ...
  return (req, res, next) => {
    ...
    jwt.verify(token, jwtConfig.secret, (err, decoded) => {
      ...
      hook.alter('tokenDecode', decoded, req, (err, decoded) => {
        ...
        cache.loadSubmission(req, formId, userId, (loadErr, user) => {
          if (loadErr) {
          ...
          hook.alter('validateToken', req, decoded, user, (validateErr) => {
            if (validateErr) {
