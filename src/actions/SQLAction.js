  ...
  class SQLAction extends Action {
    ...
    static settingsForm(req, res, next) {
      hook.settings(req, (err, settings) => {
        ...
        if (settings.databases && settings.databases.mysql) {
          missingSetting = _
            .find(['host', 'port', 'database', 'user', 'password'], (prop) => !settings.databases.mysql[prop]);
          if (!missingSetting) {
            serverTypes.push({
              type: 'mysql',
              title: 'MySQL',
        ...
        if (settings.databases && settings.databases.mssql) {
          missingSetting = _
            .find(['host', 'port', 'database', 'user', 'password'], (prop) => !settings.databases.mssql[prop]);
          if (!missingSetting) {
            serverTypes.push({
              type: 'mssql',
    ...
    resolve(handler, method, req, res, next) {
      ...
      hook.settings(req, (err, settings) => {
        ...
        const missingSetting = _.find(['host', 'port', 'database', 'user', 'password'], (prop) => !settings[prop]);
        ...
        const onSubmission = function (submission) {
          ...
          if (this.settings.type === 'mssql') {
            const config = {
              ...
              options: { encrypt: !!settings.azure },
            };
