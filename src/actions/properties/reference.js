...
module.exports = (router) => {
  ...
  const checkAccess = function (component, query, req, res) {
    return new Promise((resolve, reject) => {
      ...
      async.series(
        router.formio.resources.submission.handlers.beforeIndex.map((fn) => async.apply(fn, sub.req, sub.res)),
        (err) => {
          if (err) {
            return reject(err);
          }
        ...
        return new Promise((resolve, reject) => {
          router.formio.cache.loadForm(req, null, formId, (err, form) => {
            ...
            util.removeProtectedFields(
              form,
              'index',
              res.resource.item.map((submission) => _.get(submission, `data.${path}`)),
            );
            resolve();
          });
        });
      case 'beforePost':
        return setResource(component, path, req);
      case 'afterPost':
