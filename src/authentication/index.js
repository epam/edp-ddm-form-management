...
module.exports = (router) => {
  ...
  const getTempToken = (req, res, allow, expire, admin, cb) => {
    ...
    jwt.sign(tempToken, jwtConfig.secret, {
      ...
      return hook.alter('tempToken', req, res, allow, expire, tokenResponse, (tempTokenErr) => {
        if (tempTokenErr) {
          return cb(tempTokenErr);
        }
  ...
  const tempToken = (req, res) => {
    ...
    return getTempToken(req, res, allow, expire, adminKey, (err, token) => {
      if (err) {
  ...
  const authenticate = (req, forms, userField, passField, username, password, next) => {
    ...
    submissionModel.findOne(hook.alter('submissionQuery', query, req)).lean().exec((userErr, user) => {
      if (userErr) {
        return next(userErr);
      }
      if (!user) {
      ...
      bcrypt.compare(password, hash, (compareErr, value) => {
        if (compareErr) {
          audit('EAUTH_BCRYPT', {
            ...req,
            userId: user._id,
        ...
        router.formio.resources.form.model.findOne({
          ...
          return hook.alter('user', user, (userError, _user) => {
            if (userError) {
              // Attempt to fail safely and not update the user reference.
              debug.authenticate(userError);
            } else {
            ...
            hook.alter('login', user, req, (loginError) => {
              if (loginError) {
              ...
              return hook.alter('tokenDecode', token, req, (tokenDecodeError, decoded) => {
                // Continue with the token data.
                next(tokenDecodeError, {
                  user,
                  token: {
