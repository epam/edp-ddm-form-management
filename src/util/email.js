...
const { EMAIL_OVERRIDE } = process.env;
  ...
  const send = (req, res, message, params, next) => {
    ...
    hook.settings(req, (err, settings) => { // eslint-disable-line max-statements
      ...
      switch (emailType) {
        ...
        case 'smtp':
          if (_.has(settings, 'email.smtp')) {
            ...
            if (_.has(settings, 'email.smtp.port')) {
              _settings.port = parseInt(_.get(settings, 'email.smtp.port'), 10);
            }
        ...
        case 'test':
        default:
          transporter.sendMail = (mail, cb) => cb(null, mail);
          break;
      }
      ...
      nunjucksInjector(mail, options)
        .then((email) => {
          ...
          if (message.sendEach === true) {
            ...
            emails = addresses.map((address) => ({ ...email, to: address }));
          } else {
            emails = [email];
          }
          ...
          return chunks.reduce((result, chunk) => result.then(() => new Promise((resolve) => {
            ...
            setImmediate(
              () => Promise.all(chunk.map((email) => {
                ...
                return new Promise((resolve, reject) => hook.alter('email', email, req, res, params, (err, email) => {
                  if (err) {
                    return reject(err);
                  }
