...
const Utils = {
  ...
  ensureIds(data) {
    ...
    _.each(data, (value, key) => {
      ...
      if (_.isArray(value)) {
        changed = value.reduce((subchanged, row) => Utils.ensureIds(row) || subchanged, false) || changed;
  ...
  removeProtectedFields(form, action, submissions) {
    ...
    submissions.forEach((submission) => modifyFields.forEach((modifyField) => modifyField(submission)));
  },
