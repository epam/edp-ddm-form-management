...
module.exports = (router) => {
  ...
  const roleMachineNameToId = (template, entity) => {
    ...
    if (!(entity instanceof Array)) {
      if (entity.hasOwnProperty('role')) {
        ...
        if (template.roles.hasOwnProperty(entity.role)) {
  ...
  const entities = {
    ...
    resource: {
      ...
      cleanUp: (template, resources, done) => {
        const { model } = formio.resources.form;
   ...
   * @returns {*}
   */
  const cleanUp = (data, template, done) => async
    .series(data.map(({ entity, forms }) => async.apply(entity.cleanUp, template, forms)), (err) => {
      if (err) {
        debug.template(err);
        return done(err);
      }
  ...
  const alterFormSave = (forms, alter) => Object.values(forms || {})
    .map((form) => async.apply((done) => alter(form, done)));
