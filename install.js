...
module.exports = function installModule(formio, items, done) {
   ...
   * @returns {*}
   */
  const download = (url, zipFile, dir, done) => {
   ...
   * @returns {*}
   */
  const extract = (zipFile, fromDir, dir, done) => {
  ...
  const steps = {
    ...
    importTemplate(done) {
      ...
      const importer = importerModule({ formio });
